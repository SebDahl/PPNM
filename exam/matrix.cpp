...
void vector::append(const vector& a){
	FOR_V(i,a) data.push_back(a.data[i]);
}

double vector::norm() const {
	double sum = 0;
	FOR_V(i,SELF) sum += SELF[i] * SELF[i];
	return std::sqrt(sum);
}
...
// pp::vector matrix::back() const {
// 	if (sizerow() == 0 || sizecol() == 0) throw std::out_of_range("Matrix is empty");
// 	pp::vector result; result.resize(sizecol());
// 	for (int j = 0; j < sizecol(); j++) {
// 		result[j] = SELF[sizerow() - 1, j];
// 	}
// 	return result;
// }
...
bool has_value(const matrix& A, double tol) {
	for (int i = 0; i < A.sizerow(); i++) {
		for (int j = 0; j < A.sizecol(); j++) {
			if (std::abs(A(i, j)) > tol) return true;
		}
	}
	return false;
}
bool has_value(const vector& a, double tol) {
	for (int i = 0; i < a.size(); i++) {
		if (std::abs(a[i]) > tol) return true;
	}
	return false;
}
